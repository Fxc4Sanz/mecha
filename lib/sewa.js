    ...
    setInterval(() => {
        ...
        if (position !== null) {
            ...
            if (_dir[position].status === true){
                surya.sendMessage(_dir[position].id, `Waktu sewa di grup ini sudah habis, bot akan keluar otomatis`, MessageType.text)
                .then(() => {
                    surya.groupLeave(_dir[position].id)
                    .then(() => {
