...
if (isBan) return 
if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply (`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
const bu = [`Aku memilih untuk sendiri, bukan karena menunggu yang sempurna, tetapi butuh yang tak pernah menyerah.`,
...
const cin = bu[Math.floor(Math.random() * bu.length)]
...
if (isBan) return 
if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply (`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
const pan = [` *PAda anak kecil bermain batu,
...
surya.sendMessage(from, tun, text, { quoted: msg })
limitAdd(sender, limit)
break
case 'quotes':
if (!isRegister) return sendButMessage(from, daftar1, daftar2, daftar3, {"contextInfo": {mentionedJid: [Suryaa, sender, '0@s.whatsapp.net']}, quoted: msg})
...
`_*By Surya~*_
*quote:* Yang paling melelahkan dalam hidup adalah menjadi orang yang tidak tulus.`,
`_*By Surya~*_
...
`_*By Surya~*_
*quote:* Mulai sekarang kita tidak usah membuang-buang waktu barang semenit pun untuk memikirkan orang-orang yang tidak kita sukai.`,`_*By Surya~*_*quote:* Ketika kita berhenti membuat kontribusi, kita mulai mati.`,`_*By Surya~*_*quote:* Kesalahan terbesar yang dibuat manusia dalam kehidupannya adalah terus-menerus merasa takut bahwa mereka akan melakukan kesalahan.`,`_*By Surya~*_*quote:* Janganlah menyebut orang tidak bahagia sebelum dia mati. Jangan menilai pekerjaan seseorang sebelum pekerjaannya berakhir.`,`_*By Surya~*_*quote:* Percaya pada diri sendiri adalah rahasia utama mencapai sukses.`,`_*By Surya~*_*quote:* Anda harus waspada dengan kesenangan. Pastikan bahwa Anda menikmatinya dan bukan dikendalikannya.`,
`_*By Surya~*_
